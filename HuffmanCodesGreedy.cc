 ...
 * its frequency. The algorithm builds the tree T corresponding
 * to the optimal code in a bottom-up manner. It begins with a
 * set of |C| leaves and performs a sequence of |C| - 1 “merging”
 * operations to create the final tree. The algorithm uses a
 * min-priority queue Q, keyed on the freq attribute, to identify
 * the two least-frequent objects to merge together. When we
 * merge two objects, the result is a new object whose frequency
 * is the sum of the frequencies of the two objects that were
 * merged.
 *
 * HUFFMAN(C)
 * 	n = |C|
...
auto Huffman(T (&F)[N]) {
	...
	auto cmp = [&f](auto& c1, auto& c2)
				{return f[c1->idx] > f[c2->idx];};
	std::vector<HTN::P> hp;
	for (int i = 0; i < N; ++i)
		hp.push_back(std::make_unique<HTN>(i));
	for (int i = 0; i < N - 1; ++i) {
...
void DFS(std::vector<std::string>& ct,
		HTN::P& root, std::string& s) {
